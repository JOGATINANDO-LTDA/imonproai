...
class Contact(Base):
    ...
    status: Mapped[str] = mapped_column(String(50), default="new")  # new, qualified, proposal, won, lost
    ...
    follow_ups: Mapped[list["FollowUp"]] = relationship(back_populates="contact", cascade="all, delete-orphan")
...
class Property(Base):
    ...
    property_type: Mapped[str] = mapped_column(String(50), default="apartment")  # house, apartment, land, commercial
    status: Mapped[str] = mapped_column(String(50), default="available")  # available, sold, rented, reserved
...
class Conversation(Base):
    ...
    channel: Mapped[str] = mapped_column(String(50), nullable=False)  # whatsapp, sms, email, voice, webchat
...
class Message(Base):
    ...
    content_type: Mapped[str] = mapped_column(String(50), default="text")  # text, audio, image, document, pdf
    metadata: Mapped[dict] = mapped_column(JSONB, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class FollowUp(Base):
    ...
    status: Mapped[str] = mapped_column(String(50), default="pending")  # pending, sent, failed, cancelled
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
